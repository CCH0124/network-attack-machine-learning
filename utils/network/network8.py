...
class MyLstmModel(tf.keras.Model):
    def __init__(self):
        ...
        self.dense_three = layers.Dense(8, kernel_regularizer=tf.keras.regularizers.l2(1e-01),name='dense_three') # bias_regularizer=tf.keras.regularizers.l1_l2(l1=1e-03, l2=1e-03), activity_regularizer=tf.keras.regularizers.l1_l2(l1=1e-10, l2=1e-10)
        self.avtivation = layers.Activation(tf.nn.relu, name='dense_three_activation')

        self.dense = layers.Dense(2, kernel_regularizer=tf.keras.regularizers.l2(1e-01), name='classification')
